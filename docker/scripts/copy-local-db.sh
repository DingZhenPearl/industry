...
DOCKER_MYSQL_PORT="3308"  # Docker容器内MySQL的端口
...
# 在容器中执行 SQL 文件
docker exec -i $CONTAINER_NAME mysql -u$DOCKER_MYSQL_USER -p$DOCKER_MYSQL_PASSWORD -P $DOCKER_MYSQL_PORT < $BACKUP_FILE

# 检查导入结果
echo "验证数据导入..."
docker exec -i $CONTAINER_NAME mysql -u$DOCKER_MYSQL_USER -p$DOCKER_MYSQL_PASSWORD -P $DOCKER_MYSQL_PORT -e "USE $DB_NAME; SHOW TABLES;"
