...
# 测试数据库连接
mysql -h $DB_HOST -P $DB_PORT -u $DB_USER -p$DB_PASSWORD -e "SELECT 1 as connection_test;"
...
# 检查用户表是否存在
echo "检查用户表是否存在..."
TABLE_EXISTS=$(mysql -h $DB_HOST -P $DB_PORT -u $DB_USER -p$DB_PASSWORD -e "SHOW TABLES FROM $DB_NAME LIKE 'users';" | grep -c users || true)

if [ "$TABLE_EXISTS" -gt 0 ]; then
    echo "用户表已存在"
else
    echo "用户表不存在，正在创建..."
    mysql -h $DB_HOST -P $DB_PORT -u $DB_USER -p$DB_PASSWORD $DB_NAME -e "CREATE TABLE IF NOT EXISTS users (
